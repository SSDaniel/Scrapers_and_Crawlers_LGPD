...
if response.status_code == 200:
    ...
    for h4 in h4_elements:
        ...
        # Assumindo que o conteúdo está dentro de um elemento contido em h4, como um <div> ou diretamente após em um <p>
        # Ajuste aqui para o elemento correto se for diferente de <div> ou <p>
        container = h4.find_next_sibling('div') or h4.find_next_sibling('p')

        if container:
            # Tenta encontrar o link para o Currículo Lattes
            link_lattes = container.find('a')
            if link_lattes and 'href' in link_lattes.attrs:
                info_docente['Lattes'] = link_lattes['href']

            # Tenta encontrar o e-mail
            if 'E-mail:' in container.text:
                try:
                    info_docente['E-mail'] = container.text.split('E-mail:')[1].split()[0].strip()
                except IndexError:
                    pass  # Em caso de estrutura inesperada, não armazenar e-mail

            # Tenta encontrar a Linha de Pesquisa
            span_linha_pesquisa = container.find('span')
